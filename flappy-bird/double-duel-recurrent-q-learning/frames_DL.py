...
class Model:
    def __init__(self, output_size, learning_rate, batch_size, name):
        ...
        with tf.variable_scope(name):
            ...
            conv2 = tf.nn.relu(conv_layer(pooling1, self.w_conv2, stride = 2))
            self.w_conv3 = tf.Variable(tf.truncated_normal([3, 3, 64, 64], stddev = 0.1))
            conv3 = tf.nn.relu(conv_layer(conv2, self.w_conv3))
            ...
            action_layer = tf.Variable(tf.random_normal([256, output_size]))
            validation_layer = tf.Variable(tf.random_normal([256, 1]))
            ...
            feed_validation = tf.matmul(tensor_validation, action_validation)
